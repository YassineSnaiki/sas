...
void top_3(list_etd list);
int rechercher_nom(list_etd list,char*nom,char *prenom);
list_etd tri_alpha_dec(list_etd  list);
list_etd tri_moyen_dec(list_etd list);
...
int main() {
	...
	do {
        ...
        printf("13. top 3\n");
        printf("14. rechercher par nom\n");
        printf("15. affichage avec un tri alphabetique.\n");
        printf("16. affichage avec un tri avec moyenne.\n");
        printf("0. Exit\n");
        ...
        switch(choix) {
            ...
            case 4:
			    afficher_tout(list);
                printf("entrer l id de l etudiant a supprime: ");
                scanf("%s", id);
			...
			break;
			case 13 :
			    printf("\n les etudiants avec les trois meilleurs notes\n\n");
				top_3(list); 
			break;	
			case 14:
			    printf("\n entrer le nom  d etudiant recherche :\n");
				scanf(" %s",nom);
			    printf("\n entrer le prenom  d etudiant recherche :\n");
				scanf(" %s",prenom);
				index = rechercher_nom(list,nom,prenom);
				if(index == -1) printf("\n etudiant n existe pas.\n");
				afficher_etd(list,list.tab[index].id);
			break;
			case 15 :
			    l = tri_alpha_dec(list);
				afficher_tout(l);
			break;
			case 16 :
			    l = tri_moyen_dec(list);
				afficher_tout(l);
			break;
			case 0:
                printf("Exiting...\n");
...
void insertion_par_defaut(list_etd * list) {
	etudiant e1;
	strcpy(e1.nom,"snaiki");strcpy(e1.prenom,"yassine");strcpy(e1.dp,"departement2");e1.dn.tm_year = 101;	e1.dn.tm_mon = 8;e1.dn.tm_mday = 29, e1.note = 10.25;
	etudiant e2;
	strcpy(e2.nom,"getudiant2_nom");strcpy(e2.prenom,"etudiant2_prenom");strcpy(e2.dp,"departement1");e2.dn.tm_year = 102;	e2.dn.tm_mon = 5;e2.dn.tm_mday = 19, e2.note = 12;
	etudiant e3;
	strcpy(e3.nom,"cetudiant3_nom");strcpy(e3.prenom,"etudiant3_prenom");strcpy(e3.dp,"departement2");e3.dn.tm_year = 102;	e3.dn.tm_mon = 2;e3.dn.tm_mday = 6,e3.note = 15.5;
	etudiant e4;
	strcpy(e4.nom,"eetudiant4_nom");strcpy(e4.prenom,"etudiant4_prenom");strcpy(e4.dp,"departement3");e4.dn.tm_year = 102;	e4.dn.tm_mon = 5;e4.dn.tm_mday = 19,e4.note = 7.75;
	etudiant e5;
	strcpy(e5.nom,"hetudiant5_nom");strcpy(e5.prenom,"etudiant5_prenom");strcpy(e5.dp,"departement3");e5.dn.tm_year = 100;	e5.dn.tm_mon = 7;e5.dn.tm_mday = 10,e5.note = 17.75;
	etudiant e6;
	strcpy(e6.nom,"betudiant6_nom");strcpy(e6.prenom,"etudiant6_prenom");strcpy(e6.dp,"departement2");e6.dn.tm_year = 102;	e6.dn.tm_mon = 5;e6.dn.tm_mday = 15,e6.note = 12;
	etudiant e7;
	strcpy(e7.nom,"detudiant7_nom");strcpy(e7.prenom,"etudiant7_prenom");strcpy(e7.dp,"departement1");e7.dn.tm_year = 99;	e7.dn.tm_mon = 11;e7.dn.tm_mday = 5,e7.note = 10;
	etudiant e8;
	strcpy(e8.nom,"fetudiant8_nom");strcpy(e8.prenom,"etudiant8_prenom");strcpy(e8.dp,"departement1");e8.dn.tm_year = 101;	e8.dn.tm_mon = 0;e8.dn.tm_mday = 13,e8.note = 9;
	etudiant e9;
    strcpy(e9.nom,"aetudiant9_nom");strcpy(e9.prenom,"etudiant9_prenom");strcpy(e9.dp,"departement3");e9.dn.tm_year = 103; e9.dn.tm_mon = 6; e9.dn.tm_mday = 24, e9.note = 11.5;
...
int rechercher_nom(list_etd list ,char * nom,char * prenom) {	
	for(int i = 0;i<list.l;i++){
		if(!strcmp(nom,list.tab[i].nom)&&!!strcmp(nom,list.tab[i].prenom)) 
		return i;
	}
	return -1;
}

void afficher_dp_etd(list_etd list , char *dp){
	int l = list.l;
	for(int i =0;i<l;i++) {
		if(!strcmp(list.tab[i].dp,dp)){
		afficher_etd(list,list.tab[i].id);
...
list_etd tri_moyen_dec(list_etd list) {
	etudiant e;
	for(int i=0;i<list.l-1;i++){
		for(int j = 0;j<list.l-i-1;j++){
			if(list.tab[j].note<list.tab[j+1].note){
				e=list.tab[j];
...
void top_3(list_etd list){
	list_etd l = tri_moyen_dec(list);
	for(int i =0;i<3;i++) {
		afficher_etd(l,l.tab[i].id);
	}
}
...
list_etd tri_alpha_dec(list_etd  list) {
	...
	for(int i=0;i<list.l-1;i++){
		for(int j = 0;j<list.l-i-1;j++){
			if(strcmp(list.tab[j].nom,list.tab[j+1].nom)<0){
				e=list.tab[j];
				list.tab[j]=list.tab[j+1];
				list.tab[j+1] = e;
			}
